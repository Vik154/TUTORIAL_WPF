...
_Некоторые свойства класса Binding:_ <br>
* _ElementName:_ имя элемента, к которому создается привязка.
* _Source:_ указывает на объект-источник, если он не является элементом управления.
* _Mode:_ режим привязки.
* _IsAsync:_ если установлено в True, то использует асинхронный режим получения данных из объекта. По умолчанию равно False
