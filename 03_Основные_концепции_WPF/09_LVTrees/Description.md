...
Логическое дерево представляет основную структуру пользовательского интерфейса. Оно точно соответствует элементам, которые были объявлены в XAML-разметке, и исключает большинство визуальных элементов, созданных WPF неявно, "под капотом", для облегчения восприятия этой древовидной структуры. <br>
Логическое дерево, при отображении своих элементов, скрывает все посторонние детали их реализации, такие как шаблоны, стили, триггеры и пр., используемые для визуализации каждого объекта, включая все необходимые рисунки, фигуры, визуальные элементы и анимации.

WPF использует логическое дерево для определения нескольких вещей, включая наследование значения свойства зависимости, разрешение ресурсов и многое другое.
