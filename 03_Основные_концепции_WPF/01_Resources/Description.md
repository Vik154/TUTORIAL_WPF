...
Обычно когда ресурс используется во множестве мест, применяется один и тот же экземпляр объекта. Такое поведение — называется разделением. Также, допустимо указывать анализатору на необходимость создания отдельного экземпляра объекта при каждом его использовании. <br>
