...
Обычно когда ресурс используется во множестве мест, применяется один и тот же экземпляр объекта. Такое поведение — называемое разделением. Тем не менее, допустимо также указывать анализатору на необходимость создания отдельного экземпляра объекта при каждом его использовании. <br>
