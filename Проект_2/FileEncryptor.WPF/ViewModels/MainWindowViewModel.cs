...
internal class MainWindowViewModel : ViewModel {
    ...
    private void OnSelectFileCommandExecuted() {
        if (!_UserDialog.OpenFile("Выбор файла для шифрования", out var file_path)) 
            return;
        
        if (file_path is null || file_path.Length == 0)
    ...
    private async void OnEncryptCommandExecuted(object p) {
        ...
        if (destination_path is null || destination_path.Length == 0)
            throw new ArgumentNullException("Файл не выбран!");
    ...
    private bool CanCancelCommandExecute() => _ProcessCancellation 
        != null && !_ProcessCancellation.IsCancellationRequested;
